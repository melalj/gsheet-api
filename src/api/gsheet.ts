...
const keys = JSON.parse(
	Buffer.from(keysEnvVar, "base64").toString("utf-8"),
) as {
	client_email: string;
	private_key: string;
};
...
router.get(
	...
	async (req: Request<SheetParams>, res: Response, next: NextFunction) => {
		try {
			// Validations
...
router.get(
	...
	async (
		req: Request<SheetParams, unknown, unknown, QueryParams>,
		res: Response,
		next: NextFunction,
	) => {
		try {
			...
			const { spreadsheetId, sheetName } = params as {
				spreadsheetId: string;
				sheetName: string;
			};
			...
			const headerRow =
				(sheetRes.data.valueRanges?.[0]?.values?.[0] as string[]) || [];
			...
			headerRow.forEach((columnName: string, columnIndex: number) => {
				columns[columnName] =
					`${sheetName}!${utils.numberToLetter(columnIndex + 1)}`;
			});
			...
			Object.keys(req.body).forEach((rowNumber) => {
				...
				Object.keys(body).forEach((k) => {
					if (!columns[k]) {
						newColCount += 1;
						columns[k] =
							`${sheetName}!${utils.numberToLetter(headerRow.length + newColCount)}`;
						data.push({
							range: `${columns[k]}1`,
							values: [[k]],
						});
...
router.delete(
	"/:spreadsheetId/:sheetName",
	async (
		req: Request<SheetParams, unknown, number[], QueryParams>,
		res: Response,
		next: NextFunction,
	) => {
		try {
...
router.post(
	"/:spreadsheetId/:sheetName",
	async (
		req: Request<SheetParams, unknown, Record<string, unknown>[], QueryParams>,
		res: Response,
		next: NextFunction,
	) => {
		try {
			...
			req.body.forEach((body) => {
				Object.keys(body).forEach((k) => {
					if (!columns[k]) {
						newColCount += 1;
						columns[k] =
							`${sheetName}!${utils.numberToLetter(columnList.length + 1)}`;
						columnList.push(k);
						data.push({
							range: `${columns[k]}1`,
							values: [[k]],
